        ...
        app.post(app.conf.reqestPath.replace('TYPE', app.path.basename(__dirname)).replace('NAME', this.name), async (req, res) => {
            ...
            */
            ...
            try {
                ...
                resdata.raw['MemoryUtilization'] = {
                    Datapoints: metricData.Datapoints.sort((a, b) => new Date(b.Timestamp) - new Date(a.Timestamp))
                };

                if (req.body.limit && resdata.raw['MemoryUtilization'].Datapoints.length > req.body.limit) {
                    resdata.raw['MemoryUtilization'].Datapoints = resdata.raw['MemoryUtilization'].Datapoints.slice(0, req.body.limit);
                }
                ...
                resdata.response['MemoryUtilization'] = {
                    Average: metricData.Datapoints.length > 0 ? totalAverage / metricData.Datapoints.length : 0,
                    Maximum: totalMax,
                    Minimum: totalMin,
                    Trend: resdata.raw['MemoryUtilization'].Datapoints.map(dp => ({
